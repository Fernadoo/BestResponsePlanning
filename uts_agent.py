...
class UniformTreeSearchAgent(MDPAgent):
    ...
    def __init__(self, label, goal,
                 ...
                 verbose=False, depth=2, node_eval='MDP', discount=0.95,
                 check_repeated_states=False,
                 nn_estimator=None,
                 sample_eval=0,
                 sample_backup=0,
                 reward_scheme=None):
        super(UniformTreeSearchAgent, self).__init__(label, goal, belief_update, soft_update, verbose)
        ...
        # whether use nn to estimate node values
        if nn_estimator:
        ...
        if reward_scheme:
            ...
            self.penalty = 2e3  # 3e4
            self.goal_reward = 1e3

        self.sample_eval = sample_eval  # for cbs eval
        self.sample_backup = sample_backup  # for tree backup
    ...
    def tree_search(self, curr_locs, prev_actions=None):
        if getattr(self, 'root', None) is None:
            ...
            # prev_actions = np.delete(prev_actions, self.label)
            oppo_a = tuple(np.delete(prev_actions, self.label))
    ...
    def evaluate(self, node_to_eval):
        ...
        if self.node_eval == 'NN':
            if node_to_eval.locations == 'EDGECONFLICT':
                return
            goal = self.goal
            locations = list(node_to_eval.locations)
            loc = [locations[self.label]] + locations[:self.label] + locations[self.label + 1:]
            obs = np.concatenate([
                goal / np.array(self.layout.shape),
                np.array(loc).reshape(-1) / np.array(self.num_agents * self.layout.shape),
                np.concatenate(beliefs_prob)
            ])
            obs2tensor, _ = self.nn_estimator.policy.obs_to_tensor(obs)
            pred_value = self.nn_estimator.policy.predict_values(obs2tensor).detach().item()
            node_to_eval.val += pred_value
            return
        ...
        if self.node_eval == 'CBS':
            ...
            node_to_eval.val += value
            return

        # Or else, appeal to MDP solvers
    ...
    def backup(self, node):
        ...
        if node.type == 'MAX':
            ...
            if self.sample_backup:
                np.random.seed(618)
                if node.locations == 'EDGECONFLICT':
                    child_rewards = list(map(lambda n: n.reward, node.children))
                    node.val = np.mean(np.array(child_values) * self.discount + np.array(child_rewards))
                    return node.val

                def a2idx(a_n):
                    idx = 0
                    for i in range(len(a_n)):
                        idx += a_n[i] * 5 ** (len(a_n) - 1 - i)
                    return idx

                beliefs_pi, beliefs_prob = node.beliefs
                estimated_val = 0
                for _ in range(self.sample_backup):
                    oppo_a = []
                    for i in range(self.num_agents):
                        if i == self.label:
                            continue
                        pi = np.random.choice(beliefs_pi[i], p=beliefs_prob[i])
                        action_dist = pi[hash(node.locations[i])]
                        a_i = np.random.choice(range(5), p=action_dist / np.sum(action_dist))
                        oppo_a.append(a_i)
                    idx = a2idx(oppo_a)
                    estimated_val += node.children[idx].val * self.discount + node.children[idx].reward
                node.val = estimated_val / self.sample_backup
                return node.val
...
class AsymmetricTreeSearch(MDPAgent):
    ...
    def __init__(self, label, goal,
                 belief_update=True, soft_update=1e-6,
                 verbose=False, discount=0.9, max_it=100, explore_c=1,
                 node_eval='HEU'):
